...
from funciones import ubicacion_ip


# genero mapas con una ubicacion inicial
ubicacion_ip()
coordenada_centrar=[-11.010444,-76.111489]
hacer_mapa(coordenada_centrar,0,0)
...
def logearse():
    ...
    if request.method =="POST": 
        # trae nombre desde registrse.html y lo mete en la variable
         ...
         # recorrer todo lo guardado  en val_email
         ...
         for e in val_email:
             ...
             if e.Correo == log_email:
                 ...
                 # si la constraseña  es igual a la digitada ingresa
                 if e.Contrasenia == log_password:
                     # se coloca el valor de 1 para aceso activado
                     e.acceso=1
                     acceso=e.acceso
                     Id=e.id
                 
                     # traigo los datos de la funcion ubicacion
                     Latitud,Longitude=ubicacion_ip()
                     # guardo los datos de latitud y longitud en variables
                     e.Latitud=Latitud
                     e.Longitude=Longitude
                     # se guarda en la base de datos y se cierra
                     db.session.commit()
                     
                     coordenada_centrar=[Latitud,Longitude]
                     hacer_mapa(coordenada_centrar,Id,acceso)
                     return render_template("index.html")
...
if __name__ == "__main__":
    ...
    @app.route('/coordenadas/<string:userInfo>',methods=['POST'])
    def coordenadas(userInfo):
        userInfo=json.loads(userInfo)
        print()
        print(f"recib {userInfo ['latitude'] }")
